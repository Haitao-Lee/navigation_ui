...
class ui_all(QMainWindow, ui_interface):
    ...
    def initLight(self):
        ...
        self.regisMarks = [self.regis_mark1, self.regis_mark2, self.regis_mark3, self.regis_mark4, self.regis_mark5, self.regis_mark6, self.regis_mark7, self.regis_mark8, self.regis_mark9, self.regis_mark10, self.regis_mark11, self.regis_mark12, self.regis_mark13, 
                           self.regis_mark14, self.regis_mark15, self.regis_mark16, self.regis_mark17, self.regis_mark18, self.regis_mark19, self.regis_mark20, self.regis_mark21, self.regis_mark22, self.regis_mark23, self.regis_mark24, self.regis_mark25]
    ...
    def setupSplitters(self):
        ...
        # self.horizonSplitter.setStyleSheet("background-color:rgb(120, 120, 120);border:1px solid white")
        # self.verticalSplitter.setStyleSheet("background-color:rgb(120, 120, 120);border:1px solid white")
        ...
        self.verticalSplitter.addWidget(self.init_box) 
        self.verticalSplitter.addWidget(self.dataWidget)
        self.verticalSplitter.addWidget(self.information)
        self.verticalSplitter.addWidget(self.light_box)
        # self.init_box.setMaximumWidth(800)
        # 隐藏原来的部件
        # self.init_box.hide()
        ...
        self.horizonSplitter.addWidget(self.visual_box)
        self.whole_layout.layout().replaceWidget(self.widget_box, self.horizonSplitter)
        self.widget_box.hide()
        self.horizonSplitter.setMouseTracking(True)
        self.verticalSplitter.setStretchFactor(0, 10)
        self.verticalSplitter.setStretchFactor(1, 80)
        self.verticalSplitter.setStretchFactor(2, 0)
        self.verticalSplitter.setStretchFactor(3, 0)
        self.verticalSplitter.setSizes([10, 80, 60, 4])
        self.horizonSplitter.setStretchFactor(0, 0)
        self.horizonSplitter.setStretchFactor(1, 10)
        self.horizonSplitter.setSizes([6, 18])
